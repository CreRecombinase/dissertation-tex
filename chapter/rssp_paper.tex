...
Unlike the marginalized likelihood without the normalized-genotype assumption, precomputing a matrix decomposition for the covariance matrix will greatly accelerate efficient computation of the likelihood
...
Two genome-wide Linkage Disequilibrium estimates were generated.  The first, which I will refer to as ``in-sample'', was generated from same samples used to simulate phenotypes and subsequently generated GWAS summary statistics.  The second set I will refer to as ``reference-panel'', which is the equally sized set of individuals not used in generating GWAS summary statistics.  
...
the recombination rate, as well an estiamte of the effective population size to improve the estimate of correlation between variants.  I implemented a standalone version of this shrinkage estimator in as R package called \texttt{LDshrink}.
...
compression) brings down the hypothetical storage requirements significantly: beyond moderate genetic distances all LD estimates are shrunk to 0.   As a final means of reducing the storage and computational burden, the LD between
variants was only estimated for variants within previously identified LD blocks, identified using the method ldetect\cite{ldetect}.

\section{Results}\label{sec:org26555b8}


\subsection{Performance of RSSp in simulations}
To estimate the effectiveness of RSSp at estimating heritability we employed a large-scale GWAS simulation using real genotypes from the UK biobank.    relative to the state of the art summary-statistics-based methods and  other summary  in contrast to ldsc and GCTA, a large sample-size simulation was used.  To make the simulations as realistic as possible, real individual-level genotypes were used.
