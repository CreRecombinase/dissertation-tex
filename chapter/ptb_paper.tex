...
Statistical fine-mapping analyses, including our own, are focused on identifying individual causal variants. Despite the best efforts, it is generally difficult to narrow down a single or few causal variants in trait-associated regions.
For example, in a high-powered GWAS of type 2 diabetes (T2D), only in a quarter of associated loci, the credible
sets contain less than 10 SNPs [33]. Current fine-mapping tools are directed towards identifying variants, whereas
identifying genes usually provides more insights. 
